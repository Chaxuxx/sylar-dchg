...
static sylar_dchg::ConfigVar<uint64_t>::ptr g_tcp_server_read_timeout =
    sylar_dchg::Config::Lookup("tcp_server.read_timeout", (uint64_t)(60 * 1000 * 2),
            "tcp server read timeout");

static sylar_dchg::Logger::ptr g_logger = SYLAR_DCHG_LOG_NAME("system");

TcpServer::TcpServer(sylar_dchg::IOManager* worker,
...
bool TcpServer::bind(sylar_dchg::Address::ptr addr, bool ssl) {
...
bool TcpServer::bind(const std::vector<Address::ptr>& addrs
    ...
    for(auto& addr : addrs) {
        ...
        if(!sock->bind(addr)) {
            SYLAR_DCHG_LOG_ERROR(g_logger) << "bind fail errno="
        ...
        if(!sock->listen()) {
            SYLAR_DCHG_LOG_ERROR(g_logger) << "listen fail errno="
                << errno << " errstr=" << strerror(errno)
                << " addr=[" << addr->toString() << "]";
            fails.push_back(addr);
            continue;
    ...
    for(auto& i : m_socks) {
        SYLAR_DCHG_LOG_INFO(g_logger) << "type=" << m_type
            << " name=" << m_name
            << " ssl=" << m_ssl
...
void TcpServer::startAccept(Socket::ptr sock) {
    while(!m_isStop) {
        ...
        if(client) {
            ...
            SYLAR_DCHG_LOG_ERROR(g_logger) << "accept errno=" << errno
