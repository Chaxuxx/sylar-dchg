...
void Logger::setFormatter(const std::string& val) {//设置格式化模板
    ...
    sylar_dchg::LogFormatter::ptr new_val(new sylar_dchg::LogFormatter(val));
...
void StdoutLogAppender::log(std::shared_ptr<Logger> logger, LogLevel::Level level, LogEvent::ptr event) {
    if(level >= m_level) {
        // MutexType::Lock lock(m_mutex);
        m_formatter->format(std::cout, logger, level, event);//把std::out当作ostream的参数了 6
    }
}
...
struct LogAppenderDefine {//整合到config 需要类成员变量的定义（被转化的） 和相应的from to lexicalcast（其实不用 用config里的就行）
    ...
    LogIniter() {
        g_log_defines->addListener([](const std::set<LogDefine>& old_value, //添加回调函数 下方三个事件 log新增修改删除
            ...
            for(auto& i : new_value) {
                ...
                sylar_dchg::Logger::ptr logger;
                if(it == old_value.end()) {
                    //新增logger
                    logger = SYLAR_DCHG_LOG_NAME(i.name);
                } else {
                    if(!(i == *it)) {
                        //修改的logger
                        logger = SYLAR_DCHG_LOG_NAME(i.name);
            ...
            for(auto& i : old_value) {
                ...
                if(it == new_value.end()) {
                    //删除logger
                    auto logger = SYLAR_DCHG_LOG_NAME(i.name);
