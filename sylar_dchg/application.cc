...
static sylar_dchg::ConfigVar<std::string>::ptr g_server_work_path =
    sylar_dchg::Config::Lookup("server.work_path"
            ,std::string("/apps/work/sylar")
            , "server work path");

static sylar_dchg::ConfigVar<std::string>::ptr g_server_pid_file =
    sylar_dchg::Config::Lookup("server.pid_file"
            ,std::string("sylar.pid")
            , "server pid file");

static sylar_dchg::ConfigVar<std::string>::ptr g_service_discovery_zk =
    sylar_dchg::Config::Lookup("service_discovery.zk"
            ,std::string("")
            , "service discovery zookeeper");


static sylar_dchg::ConfigVar<std::vector<TcpServerConf> >::ptr g_servers_conf
    = sylar_dchg::Config::Lookup("servers", std::vector<TcpServerConf>(), "http server config");
...
bool Application::init(int argc, char** argv) {
    ...
    sylar_dchg::EnvMgr::GetInstance()->addHelp("s", "start with the terminal");
    sylar_dchg::EnvMgr::GetInstance()->addHelp("d", "run as daemon");
    sylar_dchg::EnvMgr::GetInstance()->addHelp("c", "conf path default: ./conf");
    sylar_dchg::EnvMgr::GetInstance()->addHelp("p", "print help");

    bool is_print_help = false;
    if(!sylar_dchg::EnvMgr::GetInstance()->init(argc, argv)) {
        is_print_help = true;
    }

    if(sylar_dchg::EnvMgr::GetInstance()->has("p")) {
        is_print_help = true;
    }

    std::string conf_path = sylar_dchg::EnvMgr::GetInstance()->getConfigPath();
    ...
    if(sylar_dchg::FSUtil::IsRunningPidfile(pidfile)) {
        SYLAR_DCHG_LOG_ERROR(g_logger) << "server is running:" << pidfile;
        return false;
    }

    if(!sylar_dchg::FSUtil::Mkdir(g_server_work_path->getValue())) {
        SYLAR_DCHG_LOG_FATAL(g_logger) << "create work path [" << g_server_work_path->getValue()
...
bool Application::run() {
    bool is_daemon = sylar_dchg::EnvMgr::GetInstance()->has("d");
...
int Application::main(int argc, char** argv) {
    ...
    SYLAR_DCHG_LOG_INFO(g_logger) << "main";
    std::string conf_path = sylar_dchg::EnvMgr::GetInstance()->getConfigPath();
    sylar_dchg::Config::LoadFromConfDir(conf_path, true);
    {
        std::string pidfile = g_server_work_path->getValue()
                                    + "/" + g_server_pid_file->getValue();
        std::ofstream ofs(pidfile);
        if(!ofs) {
            SYLAR_DCHG_LOG_ERROR(g_logger) << "open pidfile " << pidfile << " failed";
            return false;
        }
        ofs << getpid();
    }

    m_mainIOManager.reset(new sylar_dchg::IOManager(1, true, "main"));
    m_mainIOManager->schedule(std::bind(&Application::run_fiber, this));
    m_mainIOManager->addTimer(2000, [](){
            //SYLAR_DCHG_LOG_INFO(g_logger) << "hello";
    }, true);
...
int Application::run_fiber() {
    ...
    for(auto& i : http_confs) {
        SYLAR_DCHG_LOG_DEBUG(g_logger) << std::endl << LexicalCast<TcpServerConf, std::string>()(i);

        std::vector<Address::ptr> address;
        for(auto& a : i.address) {
            size_t pos = a.find(":");
            if(pos == std::string::npos) {
                //SYLAR_DCHG_LOG_ERROR(g_logger) << "invalid address: " << a;
                address.push_back(UnixAddress::ptr(new UnixAddress(a)));
                continue;
            }
            int32_t port = atoi(a.substr(pos + 1).c_str());
            //127.0.0.1
            auto addr = sylar_dchg::IPAddress::Create(a.substr(0, pos).c_str(), port);
            if(addr) {
                address.push_back(addr);
                continue;
            }
            std::vector<std::pair<Address::ptr, uint32_t> > result;
            if(sylar_dchg::Address::GetInterfaceAddresses(result,
                                        a.substr(0, pos))) {
                for(auto& x : result) {
            ...
            auto aaddr = sylar_dchg::Address::LookupAny(a);
            if(aaddr) {
        ...
        if(!server->bind(address, fails, i.ssl)) {
            for(auto& x : fails) {
                SYLAR_DCHG_LOG_ERROR(g_logger) << "bind address fail:"
                    << *x;
            }
            _exit(0);
        }
        if(i.ssl) {
            if(!server->loadCertificates(i.cert_file, i.key_file)) {
                SYLAR_DCHG_LOG_ERROR(g_logger) << "loadCertificates fail, cert_file="
            ...
            for(auto& i : svrs) {
                ...
                for(auto& s : socks) {
                    ...
                    if(str.find("0.0.0.0") == 0) {
                        ip_and_port = sylar_dchg::GetIPv4() + ":" + std::to_string(addr->getPort());
                        break;
                    } else {
    ...
    //zksd->registerServer("blog", "chat", sylar_dchg::GetIPv4() + ":8090", "xxx");
    //zksd->queryServer("blog", "chat");
    //zksd->setSelfInfo(sylar_dchg::GetIPv4() + ":8090");
