...
static sylar_dchg::ConfigVar<std::string>::ptr g_module_path
...
bool Module::handleRequest(sylar_dchg::Message::ptr req
                           ,sylar_dchg::Message::ptr rsp
                           ,sylar_dchg::Stream::ptr stream) {
    SYLAR_DCHG_LOG_DEBUG(g_logger) << "handleRequest req=" << req->toString()
            << " rsp=" << rsp->toString() << " stream=" << stream;
    return true;
}

bool Module::handleNotify(sylar_dchg::Message::ptr notify
                          ,sylar_dchg::Stream::ptr stream) {
    SYLAR_DCHG_LOG_DEBUG(g_logger) << "handleNotify nty=" << notify->toString()
            << " stream=" << stream;
    return true;
}
...
bool Module::onConnect(sylar_dchg::Stream::ptr stream) {
    return true;
}

bool Module::onDisconnect(sylar_dchg::Stream::ptr stream) {
    return true;
}
...
bool RockModule::handleRequest(sylar_dchg::Message::ptr req
                               ,sylar_dchg::Message::ptr rsp
                               ,sylar_dchg::Stream::ptr stream) {
    auto rock_req = std::dynamic_pointer_cast<sylar_dchg::RockRequest>(req);
    auto rock_rsp = std::dynamic_pointer_cast<sylar_dchg::RockResponse>(rsp);
    auto rock_stream = std::dynamic_pointer_cast<sylar_dchg::RockStream>(stream);
    return handleRockRequest(rock_req, rock_rsp, rock_stream);
}

bool RockModule::handleNotify(sylar_dchg::Message::ptr notify
                              ,sylar_dchg::Stream::ptr stream) {
    auto rock_nty = std::dynamic_pointer_cast<sylar_dchg::RockNotify>(notify);
    auto rock_stream = std::dynamic_pointer_cast<sylar_dchg::RockStream>(stream);
...
void ModuleManager::init() {
    ...
    sylar_dchg::FSUtil::ListAllFile(files, path, ".so");
